...
import { createImpactArea, updateImpactArea } from './tools';
import { DeleteFilled, EditFilled } from '@ant-design/icons';
import { useImpactAreas } from 'dataManagement/usePlatformSettings';
...
const subTitle = 'Impact Areas';

export type ImpactAreasPayloadData = {
  name: string;
  isEnabled: boolean;
};

type ImpactAreasModalProps = {
  showImpactAreaModal: boolean;
  setShowImpactAreasModal: (arg: boolean) => void;
};

export const ImpactAreasModal = ({
  showImpactAreaModal,
  setShowImpactAreasModal,
}: ImpactAreasModalProps) => {
  const [editValue, setEditValue] = useState<GlobalListEditValueType>();
  const [showEditModal, setShowEditModal] = useState<boolean>(false);

  const { data } = useImpactAreas();

  const listWithKey = data?.impactAreas
    ? addIdAsKey(data.impactAreas)
    : undefined;
  ...
  const onAddItemFinish = (value: ImpactAreasPayloadData) => {
    const payload = value;
    createImpactArea(payload)?.then(() => form.resetFields());
  };
