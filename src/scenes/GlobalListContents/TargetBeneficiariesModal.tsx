...
import { useTargetBeneficiaries } from 'dataManagement/usePlatformSettings';
import { GlobalListEditValueType, TaxType } from './types';
import { createTargetBeneficiary, updateTargetBeneficiary } from './tools';

function addIdAsKey(data: TaxType[]): TaxType[] {
  return data.map((item) => ({ key: item.id, ...item }));
}

const subTitle = 'Target Beneficiaries';

export type TargetBeneficiaryPayloadData = {
  name: string;
  isEnabled: boolean;
};

type TargetBeneficiariesModalProps = {
  showTargetBeneficiaryModal: boolean;
  setShowTargetBeneficiaryModal: (arg: boolean) => void;
};

export const TargetBeneficiariesModal = ({
  showTargetBeneficiaryModal,
  setShowTargetBeneficiaryModal,
}: TargetBeneficiariesModalProps) => {
  const [editValue, setEditValue] = useState<GlobalListEditValueType>();
  const [showEditModal, setShowEditModal] = useState<boolean>(false);

  const { data } = useTargetBeneficiaries();

  const listWithKey = data?.taxBeneficiaries
    ? addIdAsKey(data.taxBeneficiaries)
    : undefined;
  ...
  const onAddItemFinish = (value: TargetBeneficiaryPayloadData) => {
    const payload = value;
    createTargetBeneficiary(payload)?.then(() => form.resetFields());
  };
  ...
  const onEditFinish = (value: {
    ...
    updateTargetBeneficiary(id, payload).then(() => setShowEditModal(false));
  };

  const onArchiveClick = (value: GlobalListEditValueType) => {
    const payload = { name: value.name, isEnabled: false };
    updateTargetBeneficiary(value.id, payload);
  };
  ...
  const onCancel = () => {
    setShowTargetBeneficiaryModal(false);
  };
  ...
  return (
    <ModalWithForm
      ...
      visible={showTargetBeneficiaryModal}
      onCancel={onCancel}
