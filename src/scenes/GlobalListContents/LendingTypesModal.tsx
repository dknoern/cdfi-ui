...
import { createLendingType, updateLendingType } from './tools';
import { DeleteFilled, EditFilled } from '@ant-design/icons';
import { useLendingTypes } from 'dataManagement/usePlatformSettings';
...
const subTitle = 'Lending Types';

export type LendingTypePayloadData = {
  name: string;
  isEnabled: boolean;
};

type LendingTypesModalProps = {
  showLendingTypesModal: boolean;
  setShowLendingTypesModal: (arg: boolean) => void;
};

export const LendingTypesModal = ({
  showLendingTypesModal,
  setShowLendingTypesModal,
}: LendingTypesModalProps) => {
  const [editValue, setEditValue] = useState<GlobalListEditValueType>();
  const [showEditModal, setShowEditModal] = useState<boolean>(false);

  const { data } = useLendingTypes();

  const listWithKey = data?.lendingTypes
    ? addIdAsKey(data.lendingTypes)
    : undefined;
  ...
  const onAddItemFinish = (value: LendingTypePayloadData) => {
    const payload = value;
    createLendingType(payload)?.then(() => form.resetFields());
  };
  ...
  const onCancel = () => {
    setShowLendingTypesModal(false);
  };
